...
//参数 4  读取长度
//参数 5  回复buf
int PLC300Read(int type, int DevAdder, int DBAdder, int len,char* fhBuf,int s_sock)
{
    try
    {
        ...
        if(re>0)
        {
            ...
            do{
                ...
                if(buflen>=25)
                {
                    ...
                    if(errCode){
                        MyWriteLog(plcS7Strerror(errCode));
                        return -1;
                    }
            ...
            MyWriteLog("SEND ERROR: %s",strerror(errno));
            if(EAGAIN == errno)
                goto sendTryAgain;
            return -1;
        }
...
//参数 4  读取长度
//参数 5  写入buf
int PLC300Write(int type, int DevAdder, int DBAdder, int len, char* Buf,int s_sock)
{
    try
    {
        unsigned long retlen=0;
...
int PLC300WriteData(int type, int DevAdder, int DBAdder, int len, char* Buf,int s_sock)
{
    ...
    for(int i=0; i<len; i+=writeBlock)
    {
        ...
        ret = PLC300Write(type, DevAdder+i, DBAdder, writeLen, Buf+i, s_sock);
        if(ret <= 0)
            return ret;
    }
    return len;
...
int PLC300ReadData(int type, int DevAdder, int DBAdder, int len,char* fhBuf,int s_sock)
{
    ...
    for(int i=0; i<len; i+=readBlock)
    {
        ...
        ret = PLC300Read(type, DevAdder+i, DBAdder, readLen, fhBuf+i, s_sock);
        if(ret <= 0)
            return ret;
    }
    return len;
