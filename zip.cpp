...
static char *basename1(const char *name) {
    char const *p;
...
int zip_create(const char *zipname, const char *filenames[], size_t len) {
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (!mz_zip_writer_add_file(&zip_archive, basename1(name), name, "", 0, ZIP_DEFAULT_COMPRESSION_LEVEL)) {
...
size_t zip_compress_mem_to_mem(void *pOut_buf, size_t out_buf_len, const void *pSrc_buf, size_t src_buf_len)
{
    return tdefl_compress_mem_to_mem(pOut_buf, out_buf_len, pSrc_buf, src_buf_len, 2);
}
size_t zip_uncompress_mem_to_mem(void *pOut_buf, size_t out_buf_len, const void *pSrc_buf, size_t src_buf_len)
{
    return tinfl_decompress_mem_to_mem(pOut_buf, out_buf_len, pSrc_buf, src_buf_len, 2);
}
